...
Proof.
  ...
  elim (H3 (Rabs (f2 x) / R2));
    [ idtac
      | unfold Rdiv; change (R0 < Rabs (f2 x) * / R2);
        apply Rmult_lt_0_compat;
          [ apply Rabs_pos_lt; assumption | apply Rinv_0_lt_compat; idtac ] ].
  2:exact Rlt_0_2.
  clear H3; intros alp_f2 H3.
  cut
    (forall x0:R,
      Rabs (x0 - x) < alp_f2 -> Rabs (f2 x0 - f2 x) < Rabs (f2 x) / R2).
{
  intro H4.
  cut (forall a:R, Rabs (a - x) < alp_f2 -> Rabs (f2 x) / R2 < Rabs (f2 a)).
{
  intro H5.
  cut
    (forall a:R,
      Rabs a < Rmin eps_f2 alp_f2 -> / Rabs (f2 (x + a)) < R2 / Rabs (f2 x)).
{
  ...
  elim (H (Rabs (eps * f2 x / R8)));
    [ idtac
      | unfold Rdiv; change (R0 < Rabs (eps * f2 x * / R8));
        apply Rabs_pos_lt; repeat apply prod_neq_R0;
          [ red; intro H7; rewrite H7 in H6; elim (Rlt_irrefl _ H6)
            | assumption
            | apply Rinv_neq_0_compat; discrR ] ].
  intros alp_f1d H7.
  case (Req_dec (f1 x) R0); intro.
{
  case (Req_dec l1 R0); intro.
{
(***********************************)
(*              First case         *)
(*           (f1 x)=0  l1 =0       *)
(***********************************)
  cut (R0 < Rmin eps_f2 (Rmin alp_f2 alp_f1d));
    [ intro
...
}
{
  ...
  apply Rlt_le_trans with (eps / R4 + eps / R4 + eps / R4 + eps / R4).
{
  cut (Rabs (/ f2 (x + h)) * Rabs ((f1 (x + h) - f1 x) / h - l1) < eps / R4).
{
  cut (Rabs (l1 / (f2 x * f2 (x + h))) * Rabs (f2 x - f2 (x + h)) < eps / R4).
{
  cut
...
}
{
  ...
  apply Rmult_lt_0_compat; [ assumption | apply Rinv_0_lt_compat; idtac ].
  exact Rlt_0_4.
}
}
{
...
(***********************************)
  ...
  elim (H10 (Rabs (eps * Rsqr (f2 x) / (R8 * l1)))).
{
...
{
  intro H11.
  cut (R0 < Rmin (Rmin eps_f2 alp_f1d) (Rmin alp_f2 alp_f2t2)).
{
  intro.
...
{
  replace (x + a - x) with a; [ assumption | idtac ].
  unfold Rminus.
...
{
  change (R0 < Rabs (eps * Rsqr (f2 x) / (R8 * l1))).
...
(***********************************)
  case (Req_dec l1 R0); intro.
{
  case (Req_dec l2 R0); intro.
{
  elim (H0 (Rabs (Rsqr (f2 x) * eps / (R8 * f1 x))));
    [ idtac
      | apply Rabs_pos_lt; unfold Rdiv, Rsqr; repeat rewrite Rmult_assoc;
        repeat apply prod_neq_R0 ;
          [ assumption
            | assumption
            | now apply Rgt_not_eq
            | apply Rinv_neq_0_compat; apply prod_neq_R0; discrR || (try assumption) ] ].
  intros alp_f2d H12.
  cut (R0 < Rmin (Rmin eps_f2 alp_f2) (Rmin alp_f1d alp_f2d)).
{
  intro.
...
(***********************************)
  ...
  elim (H12 (Rabs (Rsqr (f2 x) * f2 x * eps / (R8 * f1 x * l2)))).
{
  intros alp_f2c H13.
  cut (R0 < Rmin (Rmin eps_f2 alp_f2) (Rmin alp_f1d (Rmin alp_f2d alp_f2c))).
{
  intro.
  exists
...
{
  change (R0 < Rabs (Rsqr (f2 x) * f2 x * eps / (R8 * f1 x * l2))).
  apply Rabs_pos_lt.
  unfold Rsqr, Rdiv.
  ...
  case (Req_dec l2 R0); intro.
{
  ...
  elim (H11 (Rabs (eps * Rsqr (f2 x) / (R8 * l1)))).
{
  clear H11; intros alp_f2t2 H11.
  elim (H0 (Rabs (Rsqr (f2 x) * eps / (R8 * f1 x)))).
{
  intros alp_f2d H12.
  cut (R0 < Rmin (Rmin eps_f2 alp_f2) (Rmin alp_f1d (Rmin alp_f2d alp_f2t2))).
{
  intro.
  exists
    (mkposreal
...
}
{
  replace (x + a - x) with a; [ assumption | idtac ].
  unfold Rminus.
  now rewrite Rplus_comm, <- Rplus_assoc, Rplus_opp_l, Rplus_0_l.
}
}
}
}
...
{
  intros alp_f2c H13.
  elim (H12 (Rabs (eps * Rsqr (f2 x) / (R8 * l1)))).
{
  intros alp_f2t2 H14.
  cut
    (R0 <
...
{
  change (R0 < Rabs (eps * Rsqr (f2 x) / (R8 * l1))); apply Rabs_pos_lt.
  unfold Rdiv, Rsqr; rewrite Rinv_mult_distr; try discrR || assumption.
  repeat apply prod_neq_R0;
    assumption ||
      (apply Rinv_neq_0_compat; assumption) ||
...
}
{
  ...
  apply Rplus_lt_reg_l with (- Rabs (f2 a) + Rabs (f2 x) / R2).
  rewrite Rplus_assoc.
  rewrite <- double_var.
...
Lemma derivable_pt_div :
  forall (f1 f2:R -> R) (x:R),
    derivable_pt f1 x ->
    derivable_pt f2 x -> f2 x <> R0 -> derivable_pt (f1 / f2) x.
Proof.
  unfold derivable_pt.
...
Lemma derivable_div :
  forall f1 f2:R -> R,
    derivable f1 ->
    derivable f2 -> (forall x:R, f2 x <> R0) -> derivable (f1 / f2).
Proof.
...
Lemma derive_pt_div :
  forall (f1 f2:R -> R) (x:R) (pr1:derivable_pt f1 x)
    (pr2:derivable_pt f2 x) (na:f2 x <> R0),
